...
chmod 644 build/libs/"$name".jar

cp native/build-aarch64/src/"$name" build/linux-aarch64/
cp build/libs/"$name".jar build/linux-aarch64/
cp build/packr/linux-aarch64-config.json build/linux-aarch64/config.json
